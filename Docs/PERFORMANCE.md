...
| 34 | **Line-end synthesis for bare `$` rules.** Both post-33 profiles rank `$` at ~1 ms/run (line comments end at `$` in most grammars). Synthesized zero-width matches per an ICU semantics probe (`\r\n` one boundary before the `\r`; trailing newline matches twice; NEL/U+2028/29 defer). Two variants: one-append-per-window, then batched-to-window-size after the first regressed. | Two 15-pair campaigns vs step 33, hashes stable. V1: JS **−0.820812%** (95% CI −1.444592…−0.193084, 3/15), real Swift **+2.774311%** (15/15), prose control **+34.951603%** (15/15), others neutral. V2 (batched): JS **−1.967443%** (−3.868070…−0.029238, 1/15 — interval unusually wide, direction corroborates V1), TS **−0.677687%** (−1.187579…−0.165163), Swift **+2.820421%**, prose **+41.087803%**. Checksums identical throughout; 261 tests incl. a dedicated differential suite passed. | **rejected & reverted** — the ECMAScript regression reproduces across both variants and fails the no-resolved-regression gate; batching refuted the per-call-dispatch hypothesis, and the real mechanism (an interaction between synthesized zero-width sequences and end-rule scanning, not correctness — token checksums matched) remains unattributed. Probe data, both raw sample sets, and the analysis are retained in the [line-ends campaign](../Benchmarks/Results/2026-07-13-line-ends/README.md). |

| 35 | **Repository-aware, cancellable, budgeted highlighting release.** Add strict typed selection, metadata resolution, six grammars, token budgets, overlay rendering, explicit result caching, and incremental documents. These are consumer layers around the existing parser; the parser control compares the complete candidate with the restored green baseline. | Formal 15-pair A/B, no exclusions, 105/105 checksums matched and hashes stable. JavaScript **+1.437098%** (95% CI −0.626537…+3.543587), TypeScript **+0.261355%** (−0.279105…+0.804744), real Swift **+0.149437%** (−0.476185…+0.778991), both incremental latencies and the prose control neutral. SQL improved **+0.840840%** (+0.174374…+1.511740). Consumer snapshot: warm cache hit 20.2 µs, theme-only rerender 1.85 ms, 200-run rendering 529 µs, and TextKit layout/draw 240.4 ms for 40 viewports. | **adopted** — no existing parser workload has a resolved regression. Raw paired observations are retained in the [repository-aware runtime campaign](../Benchmarks/Results/2026-07-23-repository-aware-runtime/README.md); consumer/cache/render/TextKit measurements are retained in the [consumer snapshot](../Benchmarks/Results/2026-07-23-consumer-highlighting/README.md). |
